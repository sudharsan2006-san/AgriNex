...
export default function CropRecommendation() {
  ...
  return (
    <div className="p-6 pb-24">
      <BackButton />
      <h1 className="text-2xl font-black text-[#1B4332] mb-6">Crop Recommendation</h1>
      <div className="space-y-4">
        <input type="text" placeholder="Soil Type (clay/sandy)" className="w-full p-4 bg-white border border-gray-100 rounded-3xl" onChange={(e) => setInputs({...inputs, soil: e.target.value})} />
        ...
        <button onClick={handleRec} className="w-full bg-[#FF922B] text-white p-4 rounded-full font-bold uppercase shadow-lg">Recommend</button>
      </div>
      {rec && (
        <div className="mt-6 p-6 bg-white rounded-3xl shadow-sm border border-gray-100">
          <h3 className="font-black text-[#2D6A4F] text-lg mb-2">{rec.crop}</h3>
