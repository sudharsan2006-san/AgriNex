...
function RadarLayer({ timestamp }: { timestamp: number | null }) {
  const map = useMap();
  const layerRef = useRef<L.TileLayer | null>(null);

  useEffect(() => {
    if (!timestamp || !map) return;
    ...
    const layer = L.tileLayer(`https://tilecache.rainviewer.com/v2/radar/${timestamp}/256/{z}/{x}/{y}/2/1_1.png`, { 
        opacity: 0.6,
        minZoom: 2,
        maxZoom: 18,
        errorTileUrl: '', // Hide failed tiles
    });
    layer.addTo(map);
...
export default function LiveWeatherMap() {
  ...
  const fetchRadarMeta = async () => {
      try {
          ...
          const allFrames = [...res.data.radar.past, ...res.data.radar.nowcast].map((f: any) => f.time);
          setRadarTimestamp(allFrames[allFrames.length - 1]);
          setRadarMeta({ timestamp: new Date(allFrames[allFrames.length - 1] * 1000) });
      } catch (e) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          alert("Unable to get your current location or weather.");
        }
      },
      () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
        ...
        alert("Unable to refresh live weather data.");
    } finally {
        setRefreshing(false);
    }
  };
    ...
    <div className="relative w-screen h-screen">
      {location ? (
        <MapContainer center={[location.lat, location.lng]} zoom={10} className="w-full h-full" ref={mapRef}>
            <TileLayer 
                ...
                maxZoom={19}
            />
            <Marker position={[location.lat, location.lng]}><Popup>Your Current Farm Location</Popup></Marker>
            <RadarLayer timestamp={radarTimestamp} />
        </MapContainer>
      ) : (
        <div className="flex h-full items-center justify-center bg-gray-900 text-white">Loading Map...</div>
      )}
        ...
        <div className="absolute bottom-4 left-4 z-[1000] w-[calc(100vw-32px)] md:w-96 space-y-4">
          <div className="bg-white/90 p-4 rounded-xl shadow-lg">
            <div className="flex justify-between items-center mb-2">
                <h2 className="font-bold text-lg flex items-center gap-2"><Droplets size={20}/> LIVE STATUS</h2>
                <div className={`text-xs font-bold flex items-center gap-1 ${isOffline ? 'text-yellow-600' : 'text-green-600'}`}>
                    {isOffline ? '⚠️ USING CACHED DATA' : '🟢 LIVE DATA'}
                </div>
            </div>
            <div className="grid grid-cols-1 gap-2 text-sm">
                <p>Weather: {weather.current.condition}</p>
                <p>Rain Probability: {weather.current.rainProb}%</p>
                <p>Expected Rainfall: {weather.current.expectedRain} mm</p>
                <p>Temperature: {weather.current.temp}°C</p>
                <p>Wind Speed: {weather.current.windSpeed} km/h</p>
                <p className="font-bold mt-2 text-green-700">Soil Moisture: {iotData?.soilMoisture}%</p>
            ...
            <div className="mt-4 p-2 bg-green-50 rounded text-xs font-semibold text-green-800">
                <p className="flex items-center gap-1 font-bold"><Wind size={14}/> AI Farmer Alert</p>
                {aiRec ? aiRec.recommendation : <button onClick={getAiRec} className="underline">Get Recommendation</button>}
            </div>
          </div>
      ...
      {radarMeta && (
          <div className="absolute top-20 left-4 z-[1000] bg-white/90 p-2 rounded text-xs">
              <p className="font-bold text-green-600">🟢 LIVE RADAR</p>
              <p>Updated: {radarMeta.timestamp.toLocaleTimeString()}</p>
              <p>Source: RainViewer</p>
          </div>
      )}
