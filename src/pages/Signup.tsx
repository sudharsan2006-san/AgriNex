...
export default function Signup() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F7F4] flex flex-col justify-center p-6">
      <BackButton />
      <h1 className="text-3xl font-black text-[#1B4332] mb-6">{t("Create Account")}</h1>
      <form onSubmit={handleSignup} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t("Full Name")} className="w-full p-4 mb-4 border border-gray-200 rounded-2xl" required />
        ...
        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("Password")} className="w-full p-4 mb-4 border border-gray-200 rounded-2xl" required />
        <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t("Confirm Password")} className="w-full p-4 mb-4 border border-gray-200 rounded-2xl" required />
